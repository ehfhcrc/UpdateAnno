...
# Handle genes with both observations as an alias and symbol 
# by removing mappings to other mappings and maintaining the 'symbol' observation
# based on the premise that this is a curation error and the other-mapping is invalid
selfMap <- data[ ALIAS == SYMBOL ]
otherMap <- data[ ALIAS != SYMBOL ]
overlap <- intersect(selfMap$ALIAS, otherMap$ALIAS) # ~700 rows on 11/2020
toRemove <- which(data$ALIAS %in% overlap & data$ALIAS != data$SYMBOL)
data <- data[ !toRemove ]

# Some aliases still map to multiple symbols
# Select the higher HGNC ID based on the premise
# that is this work was reviewed most recently
multiMapped <- table(data$ALIAS)
multiMapped <- multiMapped[ multiMapped > 1 ]
mmData <- data[ ALIAS %in% names(multiMapped), ]
for(alias in unique(mmData$ALIAS)){
  sub <- data[ALIAS == alias]
  rowsToRm <- which(data$ALIAS == alias & data$HGNC != max(sub$HGNC))
  data <- data[-rowsToRm]
}

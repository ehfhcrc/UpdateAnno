...
Read in original GMT data from within library
```{r orig-gmt}
dt_btm <- rbindlist(lapply(orig_btm_list, data.table), idcol = "module") # change to data.table
setnames(dt_btm, "V1", "ALIAS")
```

remap the current GMT list object with latest official ALIAS2SYMBOL from org.Hs.eq.db
```{r update-gmt}
...
update the data objects held in the library
```{r update-btm-data}
